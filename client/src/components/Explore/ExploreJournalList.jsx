...
export default function ExploreJournalList() {
  ...
  const recommendedJournals = Array.isArray(data?.recommendedJournals)
    ? data.recommendedJournals
    : [];
  const recomJournals = [...recommendedJournals].reverse();
      ...
      <>
        ...
        <div className="text-center py-10">
          <p className="text-xl text-gray-500">
            There are no public journals yet.
          </p>
        </div>
      </>
    );
  ...
  return (
    <>
      {/* <div className="overflow-hidden"> */}
        <ScrollArea className="h-[calc(100vh-22px)] ">
        <DashboardNav
          SearchData={SearchData}
          setSearchData={setSearchData}
          originalData={reversedData}
        />
          <div className="px-8 mt-6">
            ...
            {
              recomJournals.length > 0 ? (<div>
              <div className="py-8 flex justify-between items-center ">
                <div className="">
                  <h2 className="text-3xl font-bold">Recommended Journals</h2>
                  <p className="text-muted-foreground">
                    These Recommendations are based on the group you are part of
                  </p>
                </div>
              </div>
              <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3 bg-gray-50 p-8 ">
                {recomJournals.map((journal) => {
                  return (
                    <div
                      className="w-full max-w-md mx-auto relative group"
                      key={journal._id}
